...
namespace WindowsService2
{
    public partial class Service1 : ServiceBase
    {
        ...
        /// <summary>
        /// Точка запуска службы.
        /// </summary>
        /// <param name="args"></param>
        protected override void OnStart(string[] args)
        {
            string jName = "AAAAAAAAAAAAAAAA";
            string jSource = "SSSSSSSSSSSSSSSS";
        ...
        private static void _aceptBody()
        {
            ...
            while (true)
            {
                if (_server.Pending())
                {
                    ...
                    _client.GetStream().Write(_buf, _offset, _available);
                    el.WriteEntry("Подключился клиент: " + _client.Client.RemoteEndPoint);
                }
                Thread.Sleep(1);
            }
        }
            ...
            while (true)
            {
                lock (_clients)
                {
                    for (int _i = 0; _i < _clients.Count; _i++)
                    {
                        ...
                        {
                            el.WriteEntry("Клиент более недоступен:" + _clients[_i].Client.RemoteEndPoint);
                ...
                for (int _i = 0; _i < _clients.Count; _i++)
                {
                    try
                    {
                        ...
                        if (_available > 0)
                        {
                            ...
                            el.WriteEntry(_clients[_i].Client.RemoteEndPoint + " > " + _data);
        ...
        private static void _sendData(object data)
        {
            ...
            try
            {
                ...
                el.WriteEntry(messageMy);
            }
            catch
            {
                el.WriteEntry("Клиент более недоступен: " + client.Client.RemoteEndPoint);
