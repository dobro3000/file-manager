    ...
    public class File : AbstractClass
    {
        ...
        public override void moveto(string newName)
        {
            for (int i = 0; i < fileList.Count; i++)
            {
                try
                {
                    System.IO.File.Move(fileList[i], newName);
                    messenger(string.Format("Обьект {0} был перемещен в {1} .", fileList[i], newName));
                }
                catch (Exception e) { messenger(e.Message); }
        ...
        public override void copyto(string newName)
        {
            for (int i = 0; i < fileList.Count; i++)
            {
                try
                {
                    FileClass.Copy(fileList[i],newName, true);
        ...
        {
            if (Messang != null)
            { Messang(this, new MyEvenArgs(name)); }
        }
