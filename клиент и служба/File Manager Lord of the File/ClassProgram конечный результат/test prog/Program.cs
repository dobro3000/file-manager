...
namespace test_prog
{
    public class TestProg
    {
        ...
        public static void Main(string[] args)
        {
            try
            {
                if (args[1].StartsWith("/create"))
                {
                    try
                    {
                        string namef = args[0].Remove(0, 5).Trim('"');
                        if (namef.Contains("."))
                        {
                            System.IO.File.Create(namef);
                            if (mesEvent != null)
                                mesEvent("Файл создан.");
                        }
                        else
                        {
                            System.IO.Directory.CreateDirectory(namef);
                            if (mesEvent != null)
                                mesEvent("Каталог создан.");
                        }
                    }
                    catch (Exception exc)
                    {
                        if (mesEvent != null)
                            mesEvent(exc.Message);
                    }
                    return;
        ...
        /// <param name="e">Сообщение у события.</param>
