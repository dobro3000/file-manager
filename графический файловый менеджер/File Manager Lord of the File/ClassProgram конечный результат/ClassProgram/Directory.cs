...
namespace FilesDirs
{
    ...
    public class Directory : AbstractClass
    {
        ...
        public override void create()
        {
            for (int i = 0; i < dirList.Count; i++)
            {
                try
                {
                    ...
                    messenger(string.Format(System.IO.Path.GetRandomFileName().Replace(".", "")));
                }
                catch (Exception e) { throw e; }
            }
        }
        ...
        {
            if (Messang != null)
            { Messang(this, new MyEvenArgs(string.Format("Объект '{0}' был изменен.", new System.IO.DirectoryInfo(name).Name))); }
        }
